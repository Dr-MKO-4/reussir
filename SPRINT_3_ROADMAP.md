...
**Duration**: 5 days  
**Objective**: AI Advanced Features + Flask Integration  
...
| New Services | 2 (AIService + FlaskClient) |
| New DTOs | 2 |
| Unit Tests | 6 |
| Integration Points | Flask API + Frontend |
...
AIService (Business Logic)
    ...
    └─→ FlaskClient [Call Flask AI API]
    ↓
FlaskClient (HTTP Client)
...
- FlaskClient: HTTP client for Flask
...
Utilities/FlaskClient.cs                 (150 lines)
...
Repositories/IFlaskRepository.cs          (80 lines)
├─ Interface for Flask communication
...
└─ FlaskClient_ConnectionFailure_ReturnsDefaultResponse
```

### Configuration (1)
```
Program.cs                               (Updated)
├─ Add IFlaskRepository registration
├─ Add IFlaskClient registration
├─ Add AIService registration
├─ Add HttpClient for Flask
└─ Add AppSettings for Flask URL
```
...
✅ GetRecommendationsAsync
   ...
   - Error case: Flask timeout → default response
...
// Mock Flask client
var mockFlaskClient = new Mock<IFlaskClient>();
mockFlaskClient
...
```csharp
// In Program.cs
var flaskUrl = builder.Configuration["FlaskApiUrl"];
...
### Non-Functional
- [x] Response time < 500ms (with Flask latency)
...
### Potential Blockers
- Flask API not responding → implement retry logic
