...
### 6. **Services/FlaskClient.cs**
- ❌ Namespace: `backend.Services` (minuscule - devrait être `Backend.Services`)
- ❌ Missing: DTOs namespace correction

### 7. **Services/AIServiceWithDatabase.cs**
- ⚠️ À SUPPRIMER (complexité inutile - on utilise Flask)
...
2. [ ] `Services/FlaskClient.cs` - Namespace minuscule
...
Phase 1: Nettoyer les Services (5 min)
  ...
  └─ Corriger FlaskClient.cs namespace
...
Phase 4: Corriger Program.cs (3 min)
  ...
  ├─ Garder AIService qui appelle Flask
  └─ Rebuild
...
- **Flask**: Garder seulement l'appel Flask dans AIService
