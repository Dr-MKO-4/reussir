...
**File**: `Services/FlaskClient.cs` (410 lines)
...
try
{
    ...
    {
        _logger.LogWarning($"Flask returned {response.StatusCode}");
...
{
    // Dependencies
    private readonly IFlaskClient _flaskClient;
        ...
        int userId, int count, string preferenceLevel, string category)
    {
        try
        {
            ...
            // 3. Call Flask client
            var recommendations = await _flaskClient.GetRecommendationsAsync(
...
public async Task GetRecommendationsAsync_WithValidRequest_ReturnsRecommendations()
{
    ...
    _mockFlaskClient.Setup(c => c.GetRecommendationsAsync(...))
...
Fallback Tests (1):
✅ WithFlaskTimeout_ReturnsDefaultResponse
...
```csharp
// Add Flask HTTP Client with configuration
...
//  - IFlaskClient → FlaskClient (HttpClient configured)
...
3. SERVICE validates & processes
   ...
   ├─ Calls FlaskClient.GetRecommendationsAsync()
   └─ Returns RecommendationResponse

4. FLASK CLIENT makes HTTP request
   ├─ Converts request to snake_case
   ├─ Makes POST to Flask: http://localhost:5000/api/recommend
   ├─ Timeout: 30 seconds
...
5. FLASK SERVICE processes
   ...
   └─ Response sent back to Flask Client
